...
export async function getCustomersByIds(ids: string[]): Promise<Customer[]> {
  try {
    ...
    return ids.map(id => customerMap.get(id)).filter((c): c is Customer => c !== undefined);
  } catch (error) {
...
export async function createCustomer(
  ...
  try {
    ...
    if (data.birthday !== undefined && data.birthday !== null) {
      customerData.birthday = data.birthday instanceof Date 
  ...
  try {
    ...
    if (data.birthday !== undefined) {
      updateData.birthday = data.birthday 
        ? (data.birthday instanceof Date ? Timestamp.fromDate(data.birthday) : data.birthday)
        : null;
    }
...
export async function searchCustomers(queryString: string): Promise<Customer[]> {
  try {
    if (!queryString.trim()) {
