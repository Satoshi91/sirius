...
export async function createDocumentAction(
  ...
  if (
    !category ||
    !["personal", "employer", "office", "government", "other"].includes(
      category
    )
  ) {
    ...
    try {
      ...
      if (user && existingProject) {
        ...
        if (title.trim() !== existingProject.title) {
          ...
          details.title = {
            oldValue: existingProject.title,
            newValue: title.trim(),
          };
        }
        if (customerId.trim() !== existingProject.customerId) {
          ...
          const oldCustomer = existingProject.customerId
            ? await getCustomer(existingProject.customerId)
            : null;
        ...
        const trimmedVisaType = visaType?.trim() || undefined;
        if (trimmedVisaType !== existingProject.visaType) {
          changedFields.push("申請予定の資格");
          details.visaType = {
            oldValue: existingProject.visaType || "",
            newValue: trimmedVisaType || "",
          };
        }
...
export async function updatePaymentStatusAction(
  ...
  try {
    ...
    return {
      error: "入金ステータスの更新に失敗しました。もう一度お試しください。",
    };
  }
}
...
export async function createTaskAction(projectId: string, formData: FormData) {
  ...
  if (
    !status ||
    !["todo", "in_progress", "awaiting_approval", "completed"].includes(status)
  ) {
    return { error: "ステータスは必須です" };
  }
...
export async function updateTaskStatusAction(
  ...
  try {
    ...
    return {
      error: "タスクのステータス更新に失敗しました。もう一度お試しください。",
    };
  }
}
...
export async function updateTaskAssigneeAction(
  ...
  try {
    ...
    return {
      error: "タスクの担当者更新に失敗しました。もう一度お試しください。",
    };
  }
}
